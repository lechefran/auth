...
func (s *Store) GetPrincipal(ctx context.Context, principalType auth.PrincipalType, principalID string) (auth.Principal, error) {
	...
	var createdAt, updatedAt, disabledAt any
	if err := row.Scan(
		&principal.Type,
		&principal.ID,
		&principal.Name,
	...
	principal.DisabledAt, err = parseNullableTime(disabledAt)
	if err != nil {
		return auth.Principal{}, err
	}
...
func (s *Store) RevokeAPIKeyWithAudit(ctx context.Context, keyID string, revokedAt time.Time, event auth.AuditEvent) (auth.APIKey, error) {
	...
	key, err := getAPIKey(ctx, tx, `WHERE id = ? FOR UPDATE`, keyID)
	if err != nil {
		return auth.APIKey{}, err
	}
...
func scanAPIKey(row scanner) (auth.APIKey, error) {
	var key auth.APIKey
	var scopes any
	var createdAt, expiresAt, revokedAt, lastUsedAt any
	if err := row.Scan(
		&key.ID,
		&key.Issuer,
		&key.Prefix,
	...
	key.ExpiresAt, err = parseNullableTime(expiresAt)
	if err != nil {
		return auth.APIKey{}, err
	}
	key.RevokedAt, err = parseNullableTime(revokedAt)
	if err != nil {
		return auth.APIKey{}, err
	}
	key.LastUsedAt, err = parseNullableTime(lastUsedAt)
	if err != nil {
		return auth.APIKey{}, err
	}
...
func decodeScopes(value any) ([]string, error) {
	var raw []byte
	...
	case string:
		raw = []byte(typed)
	default:
		return nil, fmt.Errorf("unsupported scopes value %T", value)
	}
